...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        # SM121 gate (GB10: 48 SMs, 99KB smem) — persistent_topk needs >=78 SMs
        # and its FilteredTopK fallback requires 128KB smem/block; without this
        # gate decode past ~24K context dies deterministically on GB10.
        # Ref: tonyd2wild/GLM-5.3-Flash-NVFP4-1M-KV-4x-DGX-Spark SM121-CRASH-FORENSICS.
        use_persistent_topk = (
            current_platform.is_cuda()
            and topk_tokens in (
                512,
            ...
            and torch.cuda.get_device_properties(
                torch.cuda.current_device()).multi_processor_count >= 78
        )
