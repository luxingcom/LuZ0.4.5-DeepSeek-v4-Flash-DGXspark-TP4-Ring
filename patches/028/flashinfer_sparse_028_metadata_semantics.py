...
W9-R8 patch12 (metadata-semantics parity, applied AFTER patch11):

Verdict of the 2026-08-30 builder/kernel diff against the r10d stack: the
patch11 pad is already semantically complete on 0.28.0 — the SWA builder emits
per-token ``decode_swa_lens`` that reach the kernel as ``swa_topk_lens``, and
``decode_dsv4_kernel.cuh`` masks candidates by BOTH ``abs_pos >= topk_len``
and ``slot_id == -1`` (L160-161, L431-447), so the 320 padded sentinel columns
cannot contribute attention mass. This file therefore does NOT touch the
topk_length chain; it ports the three remaining r10d SM120-path semantics
that 0.28.0 dropped (empty-prefill guard, empty-chunk skip, oversized-page
zero-copy split), each of which is a crash/slow-path hazard, not a numerics
hazard.
"""
...
# --- W9-R8 patch11 (DSV4 decode topk pad, ported from r10d) -----------------
# Root cause chain (verified 2026-08-30 on GB10/SM121, crash-20260830-062246):
#   1. DSpark K=7 non-causal draft decode needs SWA index width
#      get_dspark_swa_index_width(128, 7) = cdiv(128 + 7, 64) * 64 = 192
#      (_DSPARK_SWA_INDEX_ALIGNMENT = 64, sparse_swa.noncausal_index_width).
#   2. Official 0.28.0 feeds that 192 straight into the SM120 decode kernel;
#      flashinfer 0.6.18 lists (h, 192)/(h, 256) in _DECODE_DSV4_DISPATCH and
#      in the C++ DSV4_DISPATCH grid, so every python-side guard passes.
#   3. But the TOPK=192/256 kernel template instantiations themselves fail to
#      launch on SM120/SM121: launch_sparse_mla_decode_dsv4 returns false ->
#      "decode-dsv4 launch failed (unsupported shape or kernel error)"
#      (sparse_mla_sm120_jit_binding.cu:157). Reproduced with (h=8/16/32,
#      topk=192/256) on FI 0.6.18 + torch 2.13 AND torch 2.11 alike, while
#      topk in {128, 512, 1024} launches fine.
#   4. r10d never hits it because its caller pads the width up to a working
#      kernel width with -1 (FlashInfer's invalid-slot sentinel) fillers; the
#      per-token top-k lengths stay unchanged, so padded entries cannot
#      contribute to attention.
# Pad targets deliberately exclude 192/256: those instantiations are dead.
_FLASHINFER_DSV4_DECODE_TOPKS = (128, 512, 1024)


def _pad_to_supported_dsv4_topk(topk: int) -> int:
    """Smallest SM120 decode-dsv4 kernel width >= ``topk`` that launches."""
    for supported in _FLASHINFER_DSV4_DECODE_TOPKS:
        if topk <= supported:
            return supported
    raise ValueError(
...
class DeepseekV4FlashInferSM120Attention(DeepseekV4Attention):
    ...
    def _as_sparse_cache(kv_cache: torch.Tensor) -> torch.Tensor:
        ...
        # patch12 (r10d parity): vLLM may expose an oversized allocation page
        # for the packed SWA segment while FlashInfer's native SM120/SM121 DSv4
        # decode kernel uses 64-token SWA pages
        # (_DECODE_DSV4_PAGE_BLOCK_SIZE). Packed slots are physically
        # contiguous and indices are flattened slot IDs, so split only
        # oversized pages into zero-copy 64-token views. At the current
        # 64-token SWA spec this is a no-op; it removes the hard "no decode
        # kernel for this shape" dispatch failure if the pool ever exposes
        # oversized packed pages. Compressed C128A pages are intentionally
        # only 2 tokens (and are supported by FlashInfer's dual-cache path),
        # so leave them as-is.
        if (
    ...
    def _pad_decode_sparse_indices(indices: torch.Tensor) -> torch.Tensor:
        """Pad DSpark's non-causal SWA index width to a kernel width that
        actually launches (patch11). ``topk=192`` (the DSpark K=7 width) and
        ``topk=256`` are listed in FlashInfer's dispatch grid but their kernel
        instantiations fail to launch on SM120/SM121, so widen to the next
        entry of ``_FLASHINFER_DSV4_DECODE_TOPKS`` instead.
        """
        topk = indices.shape[-1]
        if topk in _FLASHINFER_DSV4_DECODE_TOPKS:
            return indices
        supported_topk = _pad_to_supported_dsv4_topk(topk)
        # -1 is FlashInfer's documented invalid-slot sentinel. The per-token
        # top-k lengths remain unchanged, so padded entries cannot contribute
        # to attention.
    ...
    def __init__(self, vllm_config: VllmConfig, *args, **kwargs) -> None:
        ...
        # patch11: check the width the kernel will actually receive (padded
        # per _FLASHINFER_DSV4_DECODE_TOPKS), not the raw DSpark width — the
        # dispatch table lists (h, 192/256) but those instantiations do not
        # launch on SM120/SM121.
        required_topk = _pad_to_supported_dsv4_topk(
    ...
    def _forward_sparse_impl(
        ...
        num_decode_tokens = swa_metadata.num_decode_tokens
        # patch12 (r10d parity): a prefill request always carries >= 1 token;
        # scheduling corner cases can report num_prefills > 0 with
        # num_prefill_tokens == 0, and running the prefill path then launches
        # FlashInfer with an empty query chunk (empty-tensor .max() or FFI
        # ICHECK). Guard exactly like the r10d stack.
        if swa_metadata.num_prefills > 0 and swa_metadata.num_prefill_tokens > 0:
    ...
    def _forward_decode(
        ...
        # patch11: DSpark K=7 width 192 must be widened to 512 — the TOPK=192
        # instantiation does not launch on SM120/SM121 (see module header).
    ...
    def _forward_prefill(
        ...
        for chunk_idx in range(num_chunks):
            ...
            # patch12 (r10d parity): chunked prefill can produce an empty
            # trailing chunk; skip it instead of launching FlashInfer with a
            # zero-token query.
            if q_chunk.numel() == 0:
                continue
