...
#
# =============================================================================
# W9-R8 patch 9 v2 — shm_broadcast MessageQueue handle exchange via TCPStore,
# with DETERMINISTIC PER-GROUP KEYS. Baked as
#   /usr/local/lib/python3.12/dist-packages/vllm/distributed/device_communicators/shm_broadcast.py
# (drop-in full-file replacement, same as patch9 v1; bake line in
#  Dockerfile.LuZ-0.4.5-UP28 swaps to this file).
#
# 方案（一句话）: 交换 key 从"进程内顺序计数器"改为"组身份": key =
#   f"{W9_MQ_KEY_NS|w9mq2}/{pg.group_name}/{ranks0}-{ranksN}/{n}/w{writer_rank}"
#
# 根因与证据（v12 卡死, 2026-08-30）:
#   1. 0.28.0 会在【同一个全局 rank 集 [0,1,2,3]】上创建多个 MessageQueue:
#      a) TP coordinator 的 cpu_group（parallel_state.py:519, TP MQ, writer=rank0,
#         patch8 拓扑直读下 n_local=0/n_remote=3 -> handle 形如 local None + remote tcp）;
#      b) 多节点 mp executor 的 rpc 中继（multiproc_executor.py:618,
#         nnodes_within_dp=4>1 分支, 经 get_inner_dp_world_group()=_WORLD 的 cpu_group,
#         writer=rank0 以 external_writer_handle 包裹 leader executor 直接创建的
#         MessageQueue(4, n_local=1) 句柄 -> handle 形如 local ipc + remote tcp）。
#      两组的 v1 签名同为 "0-3/4"，v1 的 _w9_mq_seq 进程内计数器把"第几次调用"当成了
#      "哪个组"——任何跨进程的次数/顺序失配都会把 A 组的 handle 投递给 B 组的槽位。
#      v12 实测（head 容器内 TCPStore 客户端）: key1(w9mq/0-3/4/1, local None+remote) 与
#      key2(w9mq/0-3/4/2, local ipc+remote) 已写入, 而 key3 有 reader 在等、永远不会有
#      writer 写 —— 即不同 rank 的调用次数/顺序不对称, 序号键必然失步。
#   2. r10d（LuZ-0.4.4, torch 2.11, 生产健康）从未失步: 它有完全相同的 create 调用结构
#      （multiproc_executor external_writer_handle 路径同样存在），但交换走
#      dist.broadcast_object_list(group=pg) —— 交换天然绑定在 PG 自己的通信子上,
#      组身份是隐式的。patch9 把交换搬进 TCPStore 全局扁平键空间后, 组身份必须显式进 key。
#   3. group_name 的跨进程一致性由 torch 自身保证: torch(>=2.4, 本镜像 2.13.0) 在
#      new_group() 时以模块级计数器为每个 ProcessGroup 起名（实测容器内: WORLD='0',
#      之后 '1','2','3'...; 各 rank 以相同顺序集体创建所有组, 计数值一致）,
#      且 torch 自己就是用该名字做组内 PrefixStore 前缀完成 rendezvous 的 —— 若各 rank
#      对同一组的名字不一致, torch 自己的通信子建立就会先崩。因此 group_name 是
#      可靠、无碰撞、与调用顺序无关的组身份。
#      TP cpu_group 与 world/inner_dp cpu_group 创建序不同 -> 名字不同 -> key 不同。
#
# 相对 v1 的附加修复:
#   * reader 轮询改为 store.wait([key], 2s) + get: v1 的 store.get() 在 key 缺失时会
#     阻塞满整个 store timeout(60s) 然后抛 DistStoreError（正是
#     w9-evidence/w9r4/w9r8-v11-boot-timeline.md 预言的 "60s DistStoreError"）。
#   * TCPStore 客户端按 (MASTER_ADDR, MASTER_PORT) 缓存复用, 不再每次交换重建连接。
#   * reader 等待期间每 30s 打一条 INFO, 指明卡在哪一个 key 上（下次现场可直接定位
#     悬停的组, 而不是只看到 "卡死"）。
#   * W9_MQ_KEY_NS（默认 w9mq2）: 换代重跑时可整体作废旧键; W9_MQ_EXCHANGE_TIMEOUT_S
#     （默认 900）: reader 等待预算。
#
# 边界说明: worker 进程 respawn 后重连"一次性 READY 握手"（writer 的 XPUB 订阅计数是
#   一次性 recv n 次, gen1 订阅者的 unsubscribe 事件也会被计入）是另一个独立缺陷,
#   本补丁不解决; patch10 的 follower 挂起守卫已在 EngineCore 层挡住了 respawn 风暴。
#
# 官方 0.28.0 参照: /tmp/sb_028.py, /tmp/ps_028_full.py;
# 实验记录: w9-evidence/w9r4/W9-R8-PATCH9-DESYNC-FIX.md
# =============================================================================
import copyreg
import functools
import io
...
def check_shm_free_space(required_bytes: int, shm_path: str = SHM_PATH) -> None:
    ...
    Args:
        required_bytes: Size in bytes of the shared-memory segment to be created.
        ...
        if name is None:
            ...
            with patch(
                ...
                try:
                    ...
                    # so the shared memory block size may be larger or equal to the requested size. The size parameter is ignored
...
def _reduce_tensor(tensor: torch.Tensor):
    ...
    if (
        ...
        try:
            ...
            # Exotic tensors with the conjugate bit don't support
            # aliasing views; let torch handle those.
            pass
        else:
...
# --- W9-R8 patch 9 v2: TCPStore-based handle exchange, keyed by GROUP
# IDENTITY (see file-header comment for scheme + evidence). v1's per-process
# sequential counter is retired: two different coordinators (TP cpu_group and
# the world/inner_dp cpu_group used by the multi-node mp executor rpc relay)
# span the SAME global rank set, so "call index" cannot identify the group.
_w9_store_client = None
_w9_store_sig = None
_w9_store_lock = threading.Lock()


def _w9_store():
    """Rendezvous TCPStore client (MASTER_ADDR:MASTER_PORT), cached per process."""
    global _w9_store_client, _w9_store_sig
    host = _os.environ.get("MASTER_ADDR", "127.0.0.1")
    port = int(_os.environ.get("MASTER_PORT", "26000"))
    sig = (host, port)
    if _w9_store_sig != sig or _w9_store_client is None:
        with _w9_store_lock:
            if _w9_store_sig != sig or _w9_store_client is None:
                from torch.distributed import TCPStore

                _w9_store_client = TCPStore(
                    host, port, is_master=False, timeout=_td(seconds=60)
                )
                _w9_store_sig = sig
    return _w9_store_client


def _w9_group_name(pg) -> "str | None":
    """torch's deterministic per-group name ('0','1','2',... at creation).

    torch increments a module-global counter on every new_group() call; the
    call order is collective (all ranks create all groups in the same order),
    so every rank agrees on every group's name. torch itself prefixes the
    group's rendezvous PrefixStore with this name, i.e. cross-rank consistency
    of group_name is a hard requirement of torch's own bootstrap.
    """
    try:
        name = pg.group_name
        if callable(name):
...
def _w9_mq_key(
    pg, writer_rank: int, global_ranks, max_chunk_bytes: int, max_chunks: int
) -> str:
    ns = _os.environ.get("W9_MQ_KEY_NS", "w9mq2")
    ranks_sig = f"{global_ranks[0]}-{global_ranks[-1]}/{len(global_ranks)}"
    name = _w9_group_name(pg)
    if name is None:
        # Dead code on torch 2.13 (new_group() always names the group).
        # Stable hash of everything locally known about the group; NOTE: two
        # distinct groups with identical ranks+params would collide here --
        # pg.group_name is what disambiguates them on real torch builds.
        payload = (
            f"{sorted(global_ranks)}/{writer_rank}/"
            f"{max_chunk_bytes}/{max_chunks}"
        ).encode()
        name = "h" + _hashlib.sha1(payload).hexdigest()[:12]
    return f"{ns}/{name}/{ranks_sig}/w{writer_rank}"


def _w9_exchange(obj, is_writer, key):
    store = _w9_store()
    if is_writer:
        store.set(key, pickle.dumps(obj))
        logger.debug("W9 mq exchange: writer published handle at %s", key)
        return obj
    timeout_s = float(_os.environ.get("W9_MQ_EXCHANGE_TIMEOUT_S", "900"))
    deadline = time.monotonic() + timeout_s
    next_warn = time.monotonic() + 30.0
    while True:
        try:
            # wait() with a short per-call timeout: v1 used a bare get(), which
            # blocks the full 60s store timeout and then RAISES DistStoreError
            # when the key is missing (the predicted "60s DistStoreError").
            store.wait([key], _td(seconds=2.0))
            raw = store.get(key)
            if raw:
                return pickle.loads(raw)
        except Exception:
            pass  # wait timeout / transient store hiccup: keep polling
        now = time.monotonic()
        if now > deadline:
            raise RuntimeError(f"W9 store exchange timeout: {key}")
        if now > next_warn:
            logger.info("W9 mq exchange: still waiting for %s", key)
            next_warn = now + 30.0
...
class MessageQueue:
    ...
    class ReadTimeoutWithWarnings:
        ...
        def should_warn(self) -> bool:
            """Returns true if it's time for a timeout warning."""
    ...
    def create_from_process_group(
        ...
        if group_rank == writer_rank:
            ...
            handle = buffer_io.export_handle()
            if isinstance(pg, ProcessGroup):
                # --- W9-R8 patch 9 v2: deterministic per-group key.
                # The writer publishes under the group's identity; the key no
                # longer depends on per-process call order or call counts.
                _key = _w9_mq_key(
                    pg, writer_rank, global_ranks, max_chunk_bytes, max_chunks
                )
                _w9_exchange(handle, True, _key)
            else:
            ...
            if isinstance(pg, ProcessGroup):
                ...
                handle = _w9_exchange(None, False, _key)  # type: ignore
            else:
