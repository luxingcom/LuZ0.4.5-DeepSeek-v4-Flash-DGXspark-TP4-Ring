...
#
# W9-R8 patch12 (draft-update non-causal guard), applied AFTER patch11.
# Base: official 0.28.0 vllm/v1/attention/backends/mla/sparse_swa.py as shipped
# in LuZ-0.4.5-UP28. Single change: update_draft_decode_metadata() now
# recomputes NON-causal DSpark rows with _compute_dspark_noncausal_swa_indices_
# kernel (block-anchored window + full query block) instead of the causal
# kernel. The 0.28.0 fused multi-step draft path only exercises this for
# AutoRegressiveSpeculator drafts (DSpark/DFlash do their own single-pass
# block forward), so this is a latent-trap fix, not a live-path change.
...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def update_draft_decode_metadata(
        ...
        # patch12: rows built for a NON-causal DSpark draft block live in the
        # dedicated noncausal buffer (width = noncausal_index_width, e.g. 192)
        # and must be recomputed with `_compute_dspark_noncausal_swa_indices_`
        # kernel` (block-anchored window [prefix_len - window, seq_len) plus
        # every query token of the block). Recomputing them with the causal
        # kernel below would silently replace non-causal indices with causal
        # ones AND widen the window to the buffer width, corrupting the draft
        # block's attention. Match build()'s branch on the actual row width.
        index_width = metadata.decode_swa_indices.shape[-1]
        if self.is_dspark and index_width != self.window_size:
