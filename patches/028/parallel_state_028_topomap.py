...
def in_the_same_node_as(
    ...
    """
    # --- W9-R8 patch 8: topology-map shortcut (official 0.28.0 torch-2.13 gloo
    # cross-node broadcast deadlocks on this 4-node ring: root returns, remotes
    # never receive). VLLM_TOPO_SAME_NODE_MAP="node_id per GLOBAL rank" (csv).
    # Computes same-node locally, zero collectives. Ref: w9-evidence/w9r4/
    # W9-R8-UP28-MIGRATION-STATUS.md + v026-era VLLM_DP_MASTER_IP fix notes.
    import os as _os
    _topo = _os.environ.get("VLLM_TOPO_SAME_NODE_MAP")
    if _topo:
        _node_ids = [int(x) for x in _topo.split(",")]
        if isinstance(pg, ProcessGroup):
            _ranks = torch.distributed.get_process_group_ranks(pg)
        else:
            _ranks = [pg.rank + i for i in range(pg.world_size)]
        _src_gid = _ranks[source_rank]
        return [_node_ids[r] == _node_ids[_src_gid] for r in _ranks]
    if isinstance(pg, ProcessGroup):
