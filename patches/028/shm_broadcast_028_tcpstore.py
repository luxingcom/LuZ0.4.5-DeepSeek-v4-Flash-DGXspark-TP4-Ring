...
# --- W9-R8 patch 9: TCPStore-based handle exchange (official 0.28.0 gloo
# broadcast_object_list deadlocks cross-node on this fabric while the minimal
# 4-node gloo test passes; root cause is vllm-init-context specific).
# Exchanges the MessageQueue handle via the rendezvous TCPStore
# (MASTER_ADDR:MASTER_PORT, proven cross-node by NCCL bootstrap).
_w9_mq_seq = [0]
...
def _w9_exchange(obj, is_writer, key):
    import pickle as _pk, time as _time
    store = _w9_store()
    if is_writer:
        store.set(key, _pk.dumps(obj))
        return obj
...
class MessageQueue:
    ...
    def create_from_process_group(
        ...
        if group_rank == writer_rank:
            ...
            if isinstance(pg, ProcessGroup):
                _w9_mq_seq[0] += 1
                _key = f"w9mq/{global_ranks[0]}-{global_ranks[-1]}/{len(global_ranks)}/{_w9_mq_seq[0]}"
