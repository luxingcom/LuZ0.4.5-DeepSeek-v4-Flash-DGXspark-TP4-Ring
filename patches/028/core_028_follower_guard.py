...
class EngineCore:
    ...
    def __init__(
        ...
        # --- W9-R8 patch 10: follower hold guard, ported verbatim from r10d
        # (LuZ-0.4.4 core.py:126-129). Official 0.28.0 lost it: follower
        # EngineCores fell into collective_rpc during _initialize_kv_caches
        # and died (AssertionError), and their gen2 respawn hangs forever on
        # the one-shot MQ READY handshake. Follower holds; leader orchestrates.
        if vllm_config.parallel_config.node_rank_within_dp != 0:
            logger.info("Follower node: holding process (leader orchestrates all workers)")
            while True:
                time.sleep(3600)
