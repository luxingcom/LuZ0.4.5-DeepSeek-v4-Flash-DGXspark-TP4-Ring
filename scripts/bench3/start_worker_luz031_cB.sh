...
# DOCS: file:///opt/_PH_INSTALL_/docs/rollback-anchors-2026-08-12.md
...
[ -n "$NODE_RANK" ] && [ -n "$VLLM_HOST_IP" ] || {
  echo "ERROR: 需指定 NODE_RANK 与 VLLM_HOST_IP (如 NODE_RANK=1 VLLM_HOST_IP=_PH_HEAD_IP_.187)" >&2
  exit 1
}
...
# ---- 回滚锚点 ----
docker inspect "$NAME" > "/opt/_PH_INSTALL_/backup/rollback_tp4-rank${NODE_RANK}.json" 2>/dev/null || true
...
# =============================================================
MODEL_DIR="/opt/_PH_INSTALL_/models/deepseek-v4-flash-0731"
GATE_OK=0
for _g in $(seq 1 60); do
  if [ -f "${MODEL_DIR}/config.json" ]; then
    ...
    case "$MODEL_REAL" in
      ...
      /data/models*)
        if mountpoint -q /data/models/deepseek-v4-flash-0731 2>/dev/null; then GATE_OK=1; fi ;;
      *) GATE_OK=1 ;;
    esac
    [ "$GATE_OK" = "1" ] && break
  fi
...
if [ "$GATE_OK" != "1" ]; then
  MPT="NFS 未挂载"
  mountpoint -q /data/models/deepseek-v4-flash-0731 2>/dev/null && MPT="NFS 已挂载(/data/models)"
  mountpoint -q /mnt/models-nfs 2>/dev/null && MPT="NFS 已挂载(/mnt/models-nfs)"
