...
#       /opt/_PH_INSTALL_/state/healthcheck-rebuild.<role>
...
if [ -z "$ROLE" ]; then
  if [ "$(hostname)" = "node01" ]; then ROLE=head; else ROLE=worker; fi
fi

SVC="vllm-tp4-worker.service"
[ "$ROLE" = "head" ] && SVC="vllm-tp4-head.service"
STATE_DIR=/opt/_PH_INSTALL_/state
mkdir -p "$STATE_DIR" 2>/dev/null || true
FAIL_STATE="$STATE_DIR/healthcheck-rebuild.$ROLE.failcnt"

# 1. 先跑只读探针
if bash /opt/_PH_INSTALL_/scripts/healthcheck_hardened.sh --role "$ROLE" --timeout 30 --grace-sec 900; then
...
echo "$NOW" > "$STATE"
echo "[healthcheck-rebuild][$ROLE] 探针失败, 触发主动重建: docker rm -f 本机 tp4-rank**"
echo 0 > "$FAIL_STATE" 2>/dev/null || true
...
docker rm -f $(docker ps -aq --filter name=tp4-rank) 2>/dev/null || true
